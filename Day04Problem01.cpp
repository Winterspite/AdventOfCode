...
void findTopGuard()
{
  ...
  for (const auto &Iter : SleepMap)
  {
    ...
    for (int Idx = 0; Idx < 60; Idx++)
    {
      ...
      minutesSleeping += Minute;
      printf("%02d ", Minute);
      if (Minute > guardMinute)
      {
        guardMinute = Minute;
        minutePlace = Idx;
      }
    ...
    if (minutesSleeping > mostMinutes)
    {
      mostMinutes = minutesSleeping;
      topMinute = guardMinute;
      topGuardID = Iter.first;
      mostPlace = minutePlace;
    }
    printf("[%03d]", minutesSleeping);
    printf("\n");
  }
